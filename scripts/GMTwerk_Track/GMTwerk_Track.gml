///@func BaseTrackActor(subject, target, <opts>)
...
function Track(_subject, _target, _time) {
	var actor = new TrackActor(_subject, _target, _time);
	for (var i = 3; i < argument_count; i += 2) {
...
function ZenosTrack(_subject, _target, _fraction) {
	var actor = new ZenosTrackActor(_subject, _target, _fraction);
	for (var i = 3; i < argument_count; i += 2) {
		variable_struct_set(actor, argument[i], argument[i+1]);
	}
	__gmtwerk_insert__(actor);
...
function StepTrack(_subject, _target, _step) {
	var actor = new StepTrackActor(_subject, _target, _step);
	for (var i = 3; i < argument_count; i += 2) {
		variable_struct_set(actor, argument[i], argument[i+1]);
	}
	__gmtwerk_insert__(actor);
...
function ChannelTrack(_subject, _target, _time, _channel) {
	var actor = new ChannelTrackActor(_subject, _target, _time, _channel);
	for (var i = 4; i < argument_count; i += 2) {
		variable_struct_set(actor, argument[i], argument[i+1]);
	}
	__gmtwerk_insert__(actor);
